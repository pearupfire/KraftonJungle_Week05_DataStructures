...
int maxHeight(BTNode *node)
{
    if (node == NULL) // 빈 트리 or 자식이 없다면
        return -1;
    
    int leftHeight = maxHeight(node->left); // 왼쪽 서브트리 높이 재귀호출
    int rightHeigt = maxHeight(node->right);
    ...
    // return (leftHeight > rightHeigt ? leftHeight : rightHeigt) + 1;
}
